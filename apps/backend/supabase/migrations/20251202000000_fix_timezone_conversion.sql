-- Fix timezone conversion in date grouping functions
-- The previous implementation used AT TIME ZONE incorrectly, causing dates to be off by one day
-- The correct approach is to use the timezone() function which properly converts timestamps
...
create or replace function get_job_dates_summary(
    ...
    timezone_name text default 'UTC'  -- User's timezone (e.g., 'America/Los_Angeles')
)
returns table(
    date_key date,
    total_count bigint,
...
-- Update list_jobs to support local timezone date filtering with correct timezone conversion
